...
<div style="border:2px solid #123B5D; background:#F4F8FB; padding:18px; border-radius:10px; margin:12px 0;">
  <div style="font-size:22px; font-weight:700; color:#123B5D;">Facilitator Handout 01</div>
  <div style="margin-top:8px;"><strong>Module Focus:</strong> teaching moves, course rhythm, facilitation logic, and implementation support</div>
  <div><strong>Best Use:</strong> open the course, prepare weekly workshops, and coach teams during design studio sessions</div>
...
<div style="border-left:4px solid #123B5D; background:#F8FAFC; padding:12px 14px; margin:14px 0;">
  <strong>Facilitator Lens</strong><br/>
...
```mermaid
flowchart TD
    A["Educational game design"] --> B["Learning goals"]
    A --> C["Gameplay goals"]
    A --> D["Teacher facilitation"]
    A --> E["Prototype and testing"]
    B --> F["Target knowledge, skill, or judgment"]
    C --> G["Core loop and feedback"]
    D --> H["Before play"]
    D --> I["During play"]
    D --> J["After play"]
    E --> K["Playtest evidence"]
    K --> L["Revision decisions"]
    L --> E
    H --> M["Scaffold entry"]
    I --> N["Observe and support"]
    J --> O["Debrief and transfer"]
```
...
```mermaid
flowchart LR
    A["Course outcomes"] --> B["Session framing"]
    B --> C["Studio activity"]
    C --> D["Peer review"]
    D --> E["Debrief"]
    E --> F["Revision log"]
    F --> C
    B --> G["Teacher prompts"]
    C --> H["Learner artifacts"]
    D --> I["Feedback evidence"]
    E --> J["Implementation decisions"]
```
