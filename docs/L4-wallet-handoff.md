...
- `GET /certificate?code=<code>` — public printable certificate view. Reads `CLAIM_CODES[code]` **without consuming it**, so the learner can print first and still redeem into a wallet after. Inline SVG QR code (via `qrcode-generator`) anchors the paper to a live URL. Browser `File → Print → Save as PDF` produces the PDF — no server-side PDF renderer is needed. After the code is redeemed or expires, the page 404s; re-printing at that point requires a fresh code.
